...
const pathToUF=Meteor.absolutePath +'/.meteor/storage/UserFiles';
...
export const UserFiles  = new FilesCollection({
  ...
  onBeforeUpload() {
    if (this.file.size <= 1024 * 1024 * 128 && /png|jpe?g/i.test(this.file.extension)) {
      return true;
    }
    return "Please upload only picture and file size smaller than 128mb " + (filesize(this.file.size));
  },
...
UserFiles.on('afterUpload', function(_fileRef) {
  if (useDropBox) {
